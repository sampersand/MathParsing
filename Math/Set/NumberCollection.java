 ...
 * @version 0.72
 * @since 0.1
 * @deprecated Overridden by {@link Group} and {@link NumberCollection};
 */
public class NumberCollection<E extends Number> extends Collection<E> implements MathObject {
    ...
    /**
     * Creates a NumberCollection based off the {@link EquationSystem} eq. Passes <code>- 10, 10, 25</code> to 
     * {@link #NumberCollection(EquationSystem,double,double,double) another constructor} as the min, max, and cStep
     * (the cardinality of the NumberCollection).
     * @param pEqSys     The {@link EquationSystem} that the NumberCollection will be based off of.
     */
    public NumberCollection(final EquationSystem pEqSys) {
        this(pEqSys, -10D, 10D, 25D);
    }

    // &#060; is less than
    /**
     * Creates a NumberCollection based off the {@link EquationSystem} eq.
     * <br>The {@link #arr1} of the NumberCollection is defined by the code:
     * <code> int pos = 0;
     * for(double i = min; i &#060; max; i += (max - min) / cStep, pos++)
     *     arr1[pos] = i;</code>
     * <br>The {@link #arr2} of the NumberCollection is defined by the code:
     * <code> int pos = 0;
     * for(double i = min; i &#060; max; i += (max - min) / cStep, pos++)
     *     arr2[pos] = pred(i, pEqSys);</code>
     * @param pEqSys    The {@link EquationSystem} that the NumberCollection will be based off of.
    ...
    /**
     * Calculates and returns the estimated value of "yhat" using the the {@link EquationSystem}
     * <code>pEqSys</code>. Passes a new {@link EquationSystem}, instantiated with the equation 
     * <code>y = pVal</code>, and then returns
     * {@link EquationSystem#eval(EquationSystem,String) a double representation of "yhat"}.
     ...
     * @param pEqSys2    The {@link EquationSystem} that will be checked for equations before <code>pEqSys2</code> is.
     ...
     * @param pArr1     The "x" array that will be used to generate the "y" array of the residule NumberCollection. Also,
     *                  The "x" array for the residule NumberCollection.
     * @param pArr2     The "y" array that will be used to generate the "y" array of the residule NumberCollection.
     * @param pEq       The Equation that the residule set will be based off of.
     * @return a NumberCollection created by the residuals of <code>pArr1</code> and <code>pArr2</code>.
     * @see <a href="https://en.wikipedia.org/wiki/Errors_and_residuals">Residuals</a>
     */
    public <T extends E> NumberCollection<E> resid(NumberCollection<T> pNC,
                                                   EquationSystem pEq) {
        verifySize(pNC);
        NumberCollection<E> resid = new NumberCollection<E>();
        for(int x = 0; x < size(); x++)
            resid.add(pred(get(x), pEq) - pArr2[x];
        return new NumberCollection<E>(resid, pArr2);
    }
     ...
     * {@link #verifySize(NumberCollection) another verifySize function}.
    ...
    /**
     * Verifies that the cardinality of {@link #arr1 pNumberCollection.arr1} and {@link #arr2 pNumberCollection.arr1} is the same.
     * Passes {@link #arr1() pNumberCollection.arr1()} and {@link #arr2() pNumberCollection.arr2()} to the
     * {@link #verifySize(double[],double[]) another verifySize function}.
     * @param pNumberCollection      The NumberCollection whose {@link #arr1} and {@link #arr2} will be be verrified that their cardinality is the
     *                  same.
     * @throws IllegalArgumentException Thrown if the cardinality of {@link #arr1() pNumberCollection.arr1()} and
     *                              {@link #arr2() pNumberCollection.arr2()} aren't equal.
     * @see <a href="https://en.wikipedia.org/wiki/Cardinality">Cardinality</a>
     */
    public void verifySize(NumberCollection pNumberCollection) throws IllegalArgumentException {
        verifySize(pNumberCollection.arr1(), pNumberCollection.arr2());
    }
     ...
     */
    public void graphe() {
        new NumberCollection(arr1, resid().arr1()).graph();
    }
    ...
    public String toString() {
        return "NumberCollection: arr1 = [" + arrToString(arr1) + "], arr2 = [" + arrToString(arr2) + "]";
    }
