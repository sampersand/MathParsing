...
public class Equation implements MathObject {
    ...
    public static String fixNode(String pEq) {
        ...
        pEq = pEq.replaceAll("^([\\wΘπ_]+)=-","$1=–"); // makes parsing better. thrown together, however.
        pEq = pEq.replaceAll("([,(\\[{<*/+^-])-","$1–"); // makes parsing better. thrown together, however.
        ...
        pEq = pEq.replaceAll("(?<![0-9.])([0-9.]+)(?!\\d|\\.)"+
                             "(?!b(?:[&^≪≫]|(?:[lr][^A-Za-zΘπ_])))"+
                             "([A-Za-hk-zΘπ_])","$1·$2"); //skips i/j b/c imaginary #s
        // (?!b[&|^|≪|≫]) is for bitwise operators.
        return pEq;
    }
